...
RSpec.describe WorkPackages::AutoCompletesController do
  ...
  describe "#work_packages" do
    ...
    describe "exact-match ranking" do
      ...
      context "in semantic mode",
              with_settings: { work_packages_identifier: "semantic" } do
        let(:mentcom_project) { create(:project, identifier: "MENTCOM") }
        let!(:mentcom_member) do
          create(:member, project: mentcom_project, principal: user, roles: [role])
        end
        let!(:exact_match) do
          create(:work_package, project: mentcom_project, updated_at: 2.days.ago, skip_semantic_id_allocation: true)
        end
        let!(:prefix_match) do
          create(:work_package, project: mentcom_project, updated_at: 1.minute.ago, skip_semantic_id_allocation: true)
        end
        let!(:exact_alias) do
          create(:work_package_semantic_alias, work_package: exact_match, identifier: "MENTCOM-5")
        end
        let!(:prefix_alias) do
          create(:work_package_semantic_alias, work_package: prefix_match, identifier: "MENTCOM-50")
        end

        before { get :index, params: { project_id: mentcom_project.id, q: "MENTCOM-5" }, format: :json }
