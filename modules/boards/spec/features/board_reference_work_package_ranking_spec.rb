...
RSpec.describe "Board 'Add existing work package' ranks exact matches first",
  ...
  # Same rigor technique as spec/features/work_packages/table/queries/id_filter_ranking_spec.rb:
  # semantic identifiers decouple which string matches from the underlying id, so creation order
  # (and hence id order) can be freely chosen adversarially. prefix_match is created first (lower
  # id) and is also more recently updated, so default id-ascending order or updatedAt-desc alone
  # would both rank it first if exact_match sorting were broken or the sortBy silently dropped.
  let!(:prefix_match) do
